<template>
  <CModal :visible="visible" @close="close" size="xl">
    <CModalHeader class="modal-header-custom">
      <CModalTitle class="modal-title-custom">자재품질 검수서 검색</CModalTitle>
    </CModalHeader>
      ...
      <div class="modal-content-wrapper">
        ...
        <div class="search-section">
          <div class="search-row-single">
            <label class="search-label">검수자</label>
            <input 
              type="text" 
            ...
            <label class="search-label">검수 일자</label>
            <input 
              type="date" 
              class="date-input" 
              v-model="insp_dt" 
            ...
            <table class="data-table-modal">
              <thead>
                <tr>
                  ...
                  <th style="width: 15%">검수 수량</th>
                  <th style="width: 15%">합격 수량</th>
                  <th style="width: 20%">검수 일자</th>
                </tr>
              </thead>
              <tbody>
                <tr 
...
const prdtSearch = async () => {
  ...
  console.log(params)
  try {
    const result = await axios.get('/api/rscQltyInspSelect', { params })
...
const selectProduct = async (prdts) => {
  const params = { rsc_qlty_insp_id: prdts.rsc_qlty_insp_id }
  try {
    const result = await axios.get('/api/rscQltyInspInferSelect', { params })
    console.log(result.data)
    emit('select', {
