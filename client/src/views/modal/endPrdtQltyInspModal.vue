    ...
    <CModalBody>
      <div class="modal-body" style="max-height: 400px; overflow-y: auto">
        ...
        <div class="d-flex gap-2 mb-3">
          ...
          <input type="text" class="form-control" placeholder="검색어 입력" v-model="ctrlValue.searchKeyword" />
        </div>
          <div class="d-flex gap-2 mb-3">
...
const prdtSearch = async () => {
  ...
  // console.log(params)
  let result = await axios.get('/api/waitingFinishedPrdt', { params }).catch((err) => console.log(err))  
...
const selectProduct = async (prdts) => {  
  ...
  let result = await axios.get('/api/waitingEndPrdtQltyDeta', { params }).catch((err) => console.log(err))
  console.log(result.data)
  emit('select', {
