      ...
      <div class="modal-body" style="max-height: 400px; overflow-y: auto">
        ...
        <div class="mb-3">         
            ...
            <div class="d-flex gap-2 mb-3">
                ...
                <button class="btn btn-secondary" style="width: 70px;" @click="prdtSearch()">검색</button>
            </div>    
        </div>
        ...
        <table class="table table-bordered table-hover">
          ...
          <tbody class="table table-bordered table-hover mb-0">
            ...
            <tr v-for="(prdts, i) in prdtList" :key="i" @dblclick="selectProduct(prdts)">
              ...
              <td>{{ Math.floor(prdts.infer_qy) }}</td>
...
const prdtSearch = async () => {
  ...
  console.log(params)  
  let result = await axios.get('/api/endPrdtQltyInspSearch', { params }).catch((err) => console.log(err))
...
const selectProduct = async  (prdts) => {  
  ...
  let result = await axios.get('/api/waitingPrdtQltyDeta', { params }).catch((err) => console.log(err))
  console.log(result.data)
  emit('select', {
