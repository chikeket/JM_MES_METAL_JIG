    ...
    <div class="search-filter-box mb-2">
      <div class="row g-3 align-items-end">
        ...
        <div class="col-12 col-md-3">
          ...
          <input v-model="filters.wrhous_nm" type="text" class="form-control" />
        </div>
        <div class="col-12 col-md-3">
          <label class="form-label">입고일자</label>
          <div class="d-flex align-items-center gap-1">
            <input v-model="filters.in_dt_from" type="date" class="form-control" />
            <span class="mx-1">~</span>
            <input v-model="filters.in_dt_to" type="date" class="form-control" />
          </div>
        </div>
      </div>
    </div>
          ...
          <thead>
            <tr>
              ...
              <th class="th-resizable">
                LOT번호<span class="col-resizer" @mousedown="onResizerDown(1, $event)"></span>
              </th>
              <th class="th-resizable">
                자재ID<span class="col-resizer" @mousedown="onResizerDown(2, $event)"></span>
              </th>
              <th class="th-resizable">
                자재명<span class="col-resizer" @mousedown="onResizerDown(3, $event)"></span>
              </th>
              <th class="th-resizable">
                창고명<span class="col-resizer" @mousedown="onResizerDown(4, $event)"></span>
              </th>
              <th class="th-resizable">
                존명<span class="col-resizer" @mousedown="onResizerDown(5, $event)"></span>
              </th>
              <th class="th-resizable">
                수량<span class="col-resizer" @mousedown="onResizerDown(6, $event)"></span>
              </th>
              <th class="th-resizable">
                단위<span class="col-resizer" @mousedown="onResizerDown(7, $event)"></span>
              </th>
              <th class="th-resizable">
                입고일자<span class="col-resizer" @mousedown="onResizerDown(8, $event)"></span>
              </th>
              <th class="th-resizable">
                만료일자<span class="col-resizer" @mousedown="onResizerDown(9, $event)"></span>
              </th>
              <th class="th-resizable">
                비고<span class="col-resizer" @mousedown="onResizerDown(10, $event)"></span>
              </th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(r, idx) in displayedRows" :key="idx">
              ...
              <td class="text-start" :title="formatDate(r.in_dt)">{{ formatDate(r.in_dt) }}</td>
              <td class="text-start" :title="formatDate(r.exp_dt)">{{ formatDate(r.exp_dt) }}</td>
...
async function onSearch() {
  try {
    ...
    // 실제 LOT별 재고 현황 조회 API로 호출
    const { data } = await axios.get('/api/lot-stock', { params })
