...
watch(() => auth.user, (u) => {
  if (!u) { ownerName.value = ''; ownerEmpId.value = ''; return }
...
const allSelected = computed({
  get() { return rows.value.length > 0 && selectedIds.value.length > 0 && rows.value.every(r => !r.__empty && selectedIds.value.includes(r.id)) },
  set(checked) { selectedIds.value = checked ? rows.value.filter(r => !r.__empty).map(r => r.id) : [] }
})
const toggleAll = (ev) => { const checked = ev && ev.target ? !!ev.target.checked : false; allSelected.value = checked }
...
const selectedRsc = (r) => {
  ...
  rows.value.push({ id: newId, rsc_ordr_deta_id: r.rsc_ordr_deta_id ?? null, code: String(code), name, spec, unit, qty: 0, note: '' })
}

// 공급업체 모달 선택
const selectedCo = (co) => { if (!co) return; Info.value.co_nm = co.co_nm || ''; selectedCoId.value = co.co_id || null }
...
const onSelectRscOrdr = async (ordr) => {
  ...
  if (detail.length === 0 && currentOrdrId.value) {
    ...
    try {
      ...
      const { data } = await axios.get('/api/rscOrdrDeta', { params: { rsc_ordr_id: currentOrdrId.value } })
...
function startEdit(row, field) { if (!row || row.__empty) return; editing.id = row.id; editing.field = field; editDraft.value = field === 'qty' ? (row.qty ?? 0) : (row.note ?? '') }
...
function commitAndBlur(row, field, ev) { try { commitEdit(row, field) } finally { if (ev && ev.target && typeof ev.target.blur === 'function') ev.target.blur() } }
function cancelEdit() { editing.id = null; editing.field = null; editDraft.value = null }
const fmtQty = (n) => (n ?? 0).toLocaleString()

// 보기 좋게 빈 행 채우기
const MIN_DISPLAY_ROWS = 10
const displayRows = computed(() => { const arr = (rows.value || []).slice(); while (arr.length < MIN_DISPLAY_ROWS) arr.push({ __empty: true, id: '', code: '', name: '', spec: '', unit: '', qty: null, note: '' }); return arr })
...
const saveRscOrdr = async () => {
  try {
    ...
    const invalidQtyRows = candidateRows.filter(r => { 
      const n = Number(String(r.qty ?? '').toString().replace(/,/g, '').trim()); 
    ...
    const master = {
      ...
      reg_dt: (Info.value.regDate && String(Info.value.regDate).slice(0,10)) || (!currentOrdrId.value ? dateFormat(null, 'yyyy-MM-dd') : null),
      rm: Info.value.remark || null,
      rsc_ordr_nm: Info.value.ordrName1 || '',
    }
    
    const detailList = candidateRows.filter(r => r.code).map(r => ({ 
      rsc_id: r.code, 
      qy: Number(r.qty ?? 0), 
      rm: r.note ?? '', 
...
const deleteRscOrdr = async () => {
  if (!currentOrdrId.value) { alert('자재 발주서를 먼저 선택해주세요'); return }
  if (!confirm('선택한 자재 발주서를 삭제하시겠습니까?')) return
  try {
    const res = await axios.delete(`/api/rscOrdr/${encodeURIComponent(currentOrdrId.value)}`)
    if (res.data?.isSuccessed) { alert('삭제되었습니다.'); currentOrdrId.value = null; rows.value = []; Info.value.ordrName1 = ''; Info.value.co_nm = ''; Info.value.remark = '' } else { alert('삭제 실패') }
  } catch (err) { console.error(err); alert('삭제 오류') }
}

// 상세 선택 삭제
function deleteSelectedRows() {
  if (!selectedIds.value.length) { alert('선택된 행이 없습니다.'); return }
  if (!confirm('선택한 상세 행을 삭제하시겠습니까?')) return
  const toDeleteUiIds = new Set(selectedIds.value)
  const idsForDb = rows.value.filter(r => !r.__empty && toDeleteUiIds.has(r.id) && r.rsc_ordr_deta_id).map(r => r.rsc_ordr_deta_id)
  const removeLocally = () => { rows.value = rows.value.filter(r => !(toDeleteUiIds.has(r.id))); selectedIds.value = [] }
  if (idsForDb.length && currentOrdrId.value) {
    axios.post('/api/rscOrdr/deta/delete', { ids: idsForDb }).then(() => { removeLocally() }).catch((err) => { console.error(err); alert('상세 삭제 오류') })
  } else { removeLocally() }
}
...
async function reloadDetails() {
  ...
  try {
    ...
    rows.value = detail.map((d, i) => ({ id: i + 1, code: d.rsc_id, name: d.rsc_nm ?? '', spec: d.spec ?? '', unit: d.rsc_unit ?? d.unit ?? '', qty: Number(d.qy ?? 0), note: d.rm ?? '', rsc_ordr_deta_id: d.rsc_ordr_deta_id ?? null }))
